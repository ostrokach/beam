...
class PubsubMessagePublisherAndMatcher(BaseMatcher):
  ...
  def describe_mismatch(self, _, mismatch_description):
    ...
    mismatch_description.append_text("Got {:d} messages.\n".format(
        len(self._actual)))
    for key, count in (c_actual - c_expected).items():
      mismatch_description.append_text("Actual has {:d} extra: {:s}\n".format(
          count, key))
    for key, count in (c_expected - c_actual).items():
      mismatch_description.append_text("Expected has {:d} extra: {:s}\n".format(
          count, key))
